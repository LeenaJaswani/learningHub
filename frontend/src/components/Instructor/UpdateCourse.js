...
const UpdateCourse = () => {
  ...
  useEffect(()=>{
    ...
    try{
      axios.get(baseURL+"/instructor-course-detail/"+courseId)
      .then((response)=>{
          if(response.status)
          setCourseData({
            ...
            prev_featured_image:response.data.featured_image,
            featured_image:'',
...
const submitCourseForm=(e)=>{
  ...
  try{
    axios.put(baseURL+"/instructor-course-detail/"+courseId,courseFormData,{
      headers:{
    ...
    <div className="row">
      ...
      <section className='col-md-9'>
        <div className="card cv">
            <h3 className="card-header border-0 bg-transparent">Update Courses</h3>
      <form onSubmit={(e) => submitCourseForm(e)}>
                ...
                <div className=" row mb-2">
                    ...
                    <div className="col-sm-9">
                    <input type="text" className="form-control" id="title" value={courseData.title} name="title" onChange={handleChange} placeholder="Course Title"/>
                    </div>
                </div><div className="row mb-3">
                    <label htmlFor="inputDescription" className="col-sm-3 col-form-label" >Description</label>
                    <div className="col-sm-4">
                    <textarea  id="inputDescription" onChange={handleChange}cols="20" rows="2" className="form-control" value={courseData.description} name="description" placeholder='Description'></textarea>
                  </div>
                  </div>
                <div className="row mb-2" >
                    <label for="image" className="col-sm-3 col-form-label">Featured Image</label>
                    <div className="col-sm-4">
                    <input type="file" className="form-control" name="featured_image"  onChange={handleFileChange} id="image"/>
                    {courseData.prev_featured_image && 
                    <img width="300" height="200" className="rounded-circle"src={courseData.prev_featured_image} alt={courseData.title}/>}
                    </div>
                    </div>
                    ...
                    <div className="col-sm-4">
                    <textarea  onChange={handleChange} name="technologies" id="" cols="20" value={courseData.technologies} rows="2" className="form-control" placeholder='Python, PHP, JS etc.'></textarea>
                  </div>
                  </div>
